...
func (d *Dao) CacheUserLogin(userID int64) error {
	_, err := d.Cache.HSet(context.Background(), userStatusRedis, strconv.FormatInt(userID, 10), "1").Result()
	return err
}

func (d *Dao) CacheUserLogOut(userID int64) error {
	_, err := d.Cache.HDel(context.Background(), userStatusRedis, strconv.FormatInt(userID, 10)).Result()
	return err
}
...
func (d *Dao) CacheGetUserStatus(userIDs []int64) ([]im_home_proto.Enum_UserStatus, error) {
	...
	var res interface{}
	res, err := luaScript.Run(context.Background(), d.Cache, []string{userStatusRedis}, str).Result()
	if err != nil {
		return nil, err
	}
