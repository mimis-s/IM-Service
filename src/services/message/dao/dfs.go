...
// 聊天图片上传下载
func (d *Dao) UpLoadChatImage(user_1 int64, user_2 int64, messageID int64, payload []byte) error {
	userFrist := lib.MinInt64(user_1, user_2)
	userSecond := lib.MaxInt64(user_1, user_2)
	name := strconv.FormatInt(userFrist, 10) + "-" + strconv.FormatInt(userSecond, 10) + "-" + strconv.FormatInt(messageID, 10)

	return d.upLoadDFS(common_client.ENUM_DFS_PATH_ChatImage, name, payload)
}

func (d *Dao) DownLoadChatImage(user_1 int64, user_2 int64, messageID int64) ([]byte, error) {
	userFrist := lib.MinInt64(user_1, user_2)
	userSecond := lib.MaxInt64(user_1, user_2)
	name := strconv.FormatInt(userFrist, 10) + "-" + strconv.FormatInt(userSecond, 10) + "-" + strconv.FormatInt(messageID, 10)
...
// 聊天文件上传下载
func (d *Dao) UpLoadChatFile(user_1 int64, user_2 int64, messageID int64, payload []byte) error {
...
func (d *Dao) DownLoadChatFile(user_1 int64, user_2 int64, messageID int64) ([]byte, error) {
	userFrist := lib.MinInt64(user_1, user_2)
	userSecond := lib.MaxInt64(user_1, user_2)
	name := strconv.FormatInt(userFrist, 10) + "-" + strconv.FormatInt(userSecond, 10) + "-" + strconv.FormatInt(messageID, 10)
